...
ifeq ($(UNAME_S),Linux)
    CFLAGS = -g -Wall -std=c++11 -O3 -Wall -fPIC -shared -std=c++11 `python3 -m pybind11 --includes` -I /usr/include/python2.7 -I ./
...
ifeq ($(UNAME_S),Darwin)
    ...
    PROG = src/find_marker`python3-config --extension-suffix`
endif
